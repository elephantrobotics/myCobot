...
void displayScreenProtect()
{
  ...
  while(1) {
    if(digitalRead(WIO_5S_DOWN) == LOW || digitalRead(WIO_5S_UP) == LOW || digitalRead(WIO_5S_RIGHT) == LOW) {      
      tft.fillScreen(TFT_BLACK);//黑色填充屏幕
      DisplayAll();
      t_begin = millis();
      break;
...
{
  //tft.drawJpg(mycobot_start_PIC, sizeof(mycobot_start_PIC), 0, 0, 320, 240);
  //tft.drawJpg(ToDown, sizeof(ToDown), 0, 0, 12, 20);  
  //tft.pushImage(0, 0, 320, 240, (uint16_t *)RoboFlow_bmp);
